...
$platform = @{ "x86" = "x86"; "x64" = "x64" }
$solutionpath = Resolve-Path $SolutionDir

Set-ExecutionPolicy -Scope CurrentUser -Force Bypass

if ($Type -eq "free") {
	Run-MSBuild $solutionpath "xenvusb.sln" $configuration["free"] $platform[$Arch]
}
elseif ($Type -eq "checked") {
	Run-MSBuild $solutionpath "xenvusb.sln" $configuration["checked"] $platform[$Arch]
}
elseif ($Type -eq "sdv") {
	$archivepath = "xenvusb"
	...
	Run-MSBuildSDV $solutionpath "xenvusb" $configuration["sdv"] $platform[$Arch]
